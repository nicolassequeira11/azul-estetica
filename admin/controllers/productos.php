...
Editor::inst( $db, 'productos' )
	->fields(
		...
		Field::inst( 'coleccion' )
			->validator( Validate::notEmpty( ValidateOptions::inst()
			->message( 'Debe ingresar una colección' )	
		) ),
	...
	->join(
        Mjoin::inst( 'files' )
            ...
            ->fields(
                Field::inst( 'id' )
                    ->upload( Upload::inst( $_SERVER['DOCUMENT_ROOT'].'/azul/upload/__ID__.__EXTN__' )
                        ->db( 'files', 'id', array(
                        ...
                        ->validator( Validate::fileSize( 10000000, 'Files must be smaller that 500K' ) )
